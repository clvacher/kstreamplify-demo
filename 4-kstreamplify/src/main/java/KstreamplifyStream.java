...
import static contants.PackageEnricherTopics.inputTopic;
import static contants.PackageEnricherTopics.outputTopic;
import static contants.PackageEnricherTopics.packageEnricherDlqTopic;
import static contants.PackageEnricherTopics.referentialTopic;
...
public class KstreamplifyStream extends KafkaStreamsStarter {
    ...
    public String dlqTopic() {
        return packageEnricherDlqTopic().toString();
    }

    @Override
    public void topology(StreamsBuilder streamsBuilder) {
        // Stream input topic
        var inputStream = inputTopic().stream(streamsBuilder);

        // GlobalKTable for the referential data
        var globalKTable = referentialTopic().globalTable(streamsBuilder, "GLOBAL_REF_STORE");
        ...
        outputTopic().produce(enrichedStream);
    }
